...
# ── ROYAL UI STYLING (High Visibility & Luxury) ───────────────────────────────
st.markdown("""
<style>
...
/* Custom Cards */
.royal-card {
    background: #0C1A2E; border: 1px solid #1C3050; border-radius: 16px; padding: 24px; margin-bottom: 20px;
}
</style>
...
# Deployment Plan (Updated per MOM - Wedding SIP removed, reallocated to Sprint)
SIP_BREAKDOWN = [
    ...
    {"name": "Sprint Deployment (TBD)", "amt": 250000, "clr": "#C8A84B"} # Sprint focused
]
...
# ── TAB 1: OVERVIEW (Networth, Targets, Deployment) ──────────────────────────
if tab == "Overview":
    # Hero Card: Existing vs Targeted
    st.markdown(f"""
    <div style='background: linear-gradient(145deg, #0C1A2E, #112338); border: 1px solid #C8A84B40; border-radius: 20px; padding: 35px; text-align: center; margin-bottom: 25px;'>
        ...
        <p style='font-size: 13px; color: #4EAF7A; margin-top: 5px;'>Age 60 · The 5-Year Empire Sprint</p>
    </div>
    """, unsafe_allow_html=True)

    col1, col2 = st.columns([1, 1.2])
    with col1:
        st.markdown("<p style='font-size:12px; color:#C8A84B; font-weight:600; margin-bottom:15px;'>CURRENT ALLOCATION</p>", unsafe_allow_html=True)
    ...
    with col2:
        st.markdown("<p style='font-size:12px; color:#C8A84B; font-weight:600; margin-bottom:15px;'>MONTHLY DEPLOYMENT PLAN (₹7L)</p>", unsafe_allow_html=True)
...
# ── TAB 2: PORTFOLIO ──────────────────────────────────────────────────────────
    ...
    st.info("Note: Portfolio values will update dynamically once the Google Sheet link is integrated.")

# ── TAB 3: PROTECTION ─────────────────────────────────────────────────────────
elif tab == "Protection":
    st.markdown("### Existing Protection Shield")
    ...
    st.markdown("<br><p style='font-size:12px; color:#C8A84B; font-weight:600;'>PROPOSED UPGRADE: HDFC ERGO OPTIMA SECURE</p>", unsafe_allow_html=True)
    st.markdown("""
    <table style='width:100%; border-collapse:collapse; margin-top:10px; background:#0C1A2E; border:1px solid #1C3050; border-radius:12px; overflow:hidden;'>
        <tr style='background:#1C3050;'>
            <th style='padding:12px; text-align:left; font-size:12px; color:#7A9BBF;'>Plan Variant</th>
            <th style='padding:12px; text-align:right; font-size:12px; color:#7A9BBF;'>Base Cover</th>
            <th style='padding:12px; text-align:right; font-size:12px; color:#7A9BBF;'>Approx Premium (Annual)</th>
        </tr>
    ...
    <p style='font-size:10px; color:#7A9BBF; margin-top:8px;'>*Rough estimates based on age 55 (Primary) and spouse. Final quotes pending DOB and medical history.</p>
    """, unsafe_allow_html=True)
...
# ── TAB 4: ACTIONS ────────────────────────────────────────────────────────────
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("<div style='background:rgba(212,88,88,0.1); border:1px solid #D4585840; border-radius:12px; padding:20px;'>", unsafe_allow_html=True)
        st.markdown("<p style='color:#D45858; font-weight:600; font-size:12px; text-transform:uppercase;'>From Client</p>", unsafe_allow_html=True)
        st.markdown("<p style='font-size:13px; color:#EAE3D6;'>• FD maturity dates to avoid exit penalties<br>• Life Insurance policy PDFs for review<br>• Spouse medical history for final quotes</p>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
    with c2:
        st.markdown("<div style='background:rgba(78,175,122,0.1); border:1px solid #4EAF7A40; border-radius:12px; padding:20px;'>", unsafe_allow_html=True)
        st.markdown("<p style='color:#4EAF7A; font-weight:600; font-size:12px; text-transform:uppercase;'>From Vikram</p>", unsafe_allow_html=True)
        st.markdown("<p style='font-size:13px; color:#EAE3D6;'>• Side-by-side comparison of Optima Secure<br>• Asset Allocation Roadmap for ₹10 Cr target<br>• Execution of ₹25L Sprint SIP rebalancing</p>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
